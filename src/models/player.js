export class Player {
  ...
  addAttribute(attributeArray) {
    for (let i = 0; i < attributeArray.length; i++) {
      if (!(this.attributes.includes(attributeArray[i]))) {
  ...
  formatMoney(amount) {
    let isNeg;
    amount < 0 ? isNeg = '-' : isNeg = '';
    let reverseArray = Math.abs(Math.floor(amount)).toString().split('').reverse();
    let insertedCommas = (reverseArray.map((ele, ind) => ind > 0 && ind % 3 === 0 ? (ele.slice(0, ind) + ',' + ele.slice(ind)) : ele)).reverse().join('');
    return isNeg + insertedCommas;
  }
}
